...
def mostrar_contenido_csv(ruta_archivo, fecha_inicio, fecha_fin):
    contenido_text.delete(1.0, tk.END)  # Limpiar el cuadro de texto
    ...
    with open(ruta_archivo, 'r') as archivo:
        ...
        # Mostrar los encabezados en el cuadro de texto
        contenido_text.insert(tk.END, f"{'IP_NAS_AP': <15}{'Input_Octects': <15}{'Output_Octects'}\n")
        
        # Mostrar el contenido filtrado de cada fila en el cuadro de texto
        for fila in lector_csv:
            ip_nas_ap = fila[indice_ip_nas_ap]
            inicio_conexion_dia = fila[indice_inicio_conexion_dia]
            input_octets = fila[indice_input_octets]
            output_octets = fila[indice_output_octets]

            # Verificar si la fecha de inicio de conexión está dentro del rango especificado
            if re.match(r'\d{4}-\d{2}-\d{2}', inicio_conexion_dia):
                if fecha_inicio <= inicio_conexion_dia <= fecha_fin:
                    contenido_text.insert(tk.END, f"{ip_nas_ap: <15}{inicio_conexion_dia: <20}{input_octets: <15}{output_octets}\n")
...
def obtener_archivo():
    ...
    mostrar_contenido_csv(ruta_archivo, fecha_inicio_entry.get(), fecha_fin_entry.get())
...
def calcular_trafico():
    ...
    try:
        ...
        # Mostrar el contenido filtrado del archivo CSV
        mostrar_contenido_csv(ruta_archivo, fecha_inicio, fecha_fin)
...
contenido_text = tk.Text(ventana, height=10, width=60)
contenido_text.pack()

calcular_button = tk.Button(ventana, text="Calcular tráfico", command=calcular_trafico)
calcular_button.pack()

resultado_text = tk.Text(ventana, height=5, width=60)
resultado_text.pack()
